...
std::string ChassisState::ToJson() const {
  ...
  out << '{' << "\"timestamp_ms\":" << timestamp_ms << ',' << "\"source\":\""
      << json::EscapeString(source) << "\"," << "\"motion_valid\":"
      << (motion_valid ? "true" : "false") << ',' << "\"running\":"
      << (running ? "true" : "false") << ',' << "\"control_state\":"
      << static_cast<unsigned int>(control_state) << ',' << "\"left_velocity_mm_s\":"
      << left_velocity_mm_s << ',' << "\"right_velocity_mm_s\":" << right_velocity_mm_s << ','
      ...
      << "\"heartbeat_age_ms\":" << heartbeat_age_ms << ',' << "\"node_state\":"
      << static_cast<unsigned int>(node_state) << ',' << "\"uptime_ms\":" << uptime_ms << ','
      << "\"fault_summary\":" << fault_summary << ',' << "\"fault_severity\":"
      << static_cast<unsigned int>(fault_severity) << ',' << "\"active_faults\":"
      << active_faults << ',' << "\"latched_faults\":" << latched_faults << ','
