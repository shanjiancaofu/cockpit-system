...
coverage。窗口默认以可缩放的 `960×540` 显示，保持 16:9，不会按原始 `1920×1080` 撑满桌面；
预览在主线程刷新，避免 GTK 窗口创建成功但画面为空。
窗口中的状态文字使用 OpenCV ASCII 字体，详细中文动作以终端为准；按 `q` 或 `Esc` 可关闭预览
窗口。每收集一个有效候选后，按照终端提示改变棋盘的位置、倾角或距离。

`--preview` 需要当前 shell 可访问图形会话（`DISPLAY` 或 `WAYLAND_DISPLAY`）。在 SSH/headless
环境中程序会打印提示并自动回退到 CLI，不会因 GTK 无法初始化而退出；如需窗口预览，请在 Jetson
本地桌面终端或正确转发 X11/Wayland 的会话中运行。
