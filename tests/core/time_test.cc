...
int main() {
  const std::int64_t system_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                     ...
                                     .count();
  const std::int64_t wall_ms = cockpit::time::WallNowMs();
  const std::int64_t steady_before_ms = cockpit::time::SteadyNowMs();
  const std::int64_t steady_after_ms = cockpit::time::SteadyNowMs();
