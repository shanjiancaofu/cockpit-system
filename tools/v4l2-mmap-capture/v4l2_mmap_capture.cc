...
int main(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (index + 1 >= argc) { std::cerr << arg << " requires a value\n"; return 2; }
    const std::string value = argv[++index];
    try {
      if (arg == "--device") config.device = value;
      else if (arg == "--width") config.width = std::stoul(value);
      else if (arg == "--height") config.height = std::stoul(value);
      else if (arg == "--fps") config.fps = std::stoul(value);
  ...
  if (frames <= 0) { std::cerr << "frames must be positive\n"; return 2; }
  cockpit::camera::V4l2MmapCapture capture;
  std::string error;
  if (!capture.Start(config, &error)) { std::cerr << error << '\n'; return 1; }
  ...
  for (int index = 0; index < frames; ++index) {
    ...
    if (!capture.WaitFrame(&frame, 5000, &error)) { std::cerr << error << '\n'; return 1; }
    const auto path = std::filesystem::path(output_dir) / ("frame-" + std::to_string(index) + ".raw");
    std::ofstream output(path, std::ios::binary);
    output.write(reinterpret_cast<const char*>(frame.data.data()), static_cast<std::streamsize>(frame.data.size()));
    if (!output) { std::cerr << "failed to write " << path << '\n'; return 1; }
