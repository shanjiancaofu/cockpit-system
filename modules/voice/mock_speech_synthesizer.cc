...
SpeechSynthesisResult MockSpeechSynthesizer::Synthesize(
  ...
  const std::size_t duration_ms =
      std::clamp<std::size_t>(text.size() * 20U, 200U, 1200U);
  const std::size_t sample_count =
      static_cast<std::size_t>(result.audio.format.sample_rate_hz) *
      duration_ms / 1000U;
  ...
  for (std::size_t index = 0; index < sample_count; ++index) {
    const double seconds =
        static_cast<double>(index) / result.audio.format.sample_rate_hz;
    result.audio.samples[index] = static_cast<std::int16_t>(
        kAmplitude * std::sin(2.0 * kPi * kFrequencyHz * seconds));
  }
  result.success = true;
  return result;
}
