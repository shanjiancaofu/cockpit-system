...
bool AlsaPcm::Open(const std::string& device, PcmDirection direction,
  ...
  int result = snd_pcm_open(&handle_, device.c_str(), stream, 0);
  if (result < 0) {
    handle_ = nullptr;
...
bool AlsaPcm::ReadFrames(std::int16_t* samples, std::size_t frame_count,
                         std::string* error) {
  if (handle_ == nullptr || direction_ != PcmDirection::kCapture) {
    return Fail("ALSA capture device is not open", error);
  }
  if (samples == nullptr && frame_count > 0) {
    return Fail("ALSA capture buffer is null", error);
  }
  ...
  while (completed < frame_count) {
    ...
    if (result < 0) {
      if (!Recover(static_cast<int>(result), "ALSA capture failed", error)) {
        return false;
      }
      continue;
    }
    if (result == 0) {
      return Fail("ALSA capture made no progress", error);
    }
