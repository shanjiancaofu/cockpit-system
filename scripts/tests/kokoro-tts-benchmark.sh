...
if [[ "$(cockpit_native_arch)" != "x86_64" ]]; then
  echo "Kokoro TTS benchmark requires x86_64" >&2
  exit 2
fi
...
repetitions="${COCKPIT_KOKORO_BENCHMARK_REPETITIONS:-3}"
...
for required in \
  ...
  if [[ ! -f "${required}" ]]; then
    echo "missing required Kokoro benchmark resource: ${required}" >&2
    exit 2
  fi
done
...
COCKPIT_AI_ROOT="${ai_root}" \
  ...
  "${build_dir}/bin/sherpa_tts_benchmark_test" "${repetitions}"
