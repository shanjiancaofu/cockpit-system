...
GatewayVehicleStatusClient::GatewayVehicleStatusClient(
...
bool GatewayVehicleStatusClient::GetLatest(VehicleStatusSnapshot* status,
                                           std::string* error) {
  ...
  const grpc::Status rpc_status =
      stub_->GetLatestVehicleState(&context, request, &response);
  if (!rpc_status.ok()) {
    if (error != nullptr) {
