...
```text
          ...
          ┌───────────────────┼───────────────────┐
          │                   │                   │
      synthetic          Argus/GStreamer       Direct V4L2
                              │                   │
                    nvarguscamerasrc/ISP      ioctl/MMAP/poll
                              │                   │
                         BGRx/RGB              RG10 Bayer RAW
                                                  │
                                             Software ISP
                                      BLC/WB/demosaic/gamma
                                                  │
                                                BGRx
```

- `gstreamer`/`argus` 是 Jetson 正式相机链路：`nvarguscamerasrc` 调用 NVIDIA Camera Stack 和硬件 ISP。
- `gstreamer` 对严格的 `/dev/video<N>` 路径使用 `v4l2src`，用于普通 USB YUYV/MJPEG 摄像头；不改变 Direct V4L2 RAW backend 的职责。
- `v4l2` 是 Direct V4L2 userspace 链路：`V4l2MmapCapture` 负责 `open/ioctl/MMAP/poll/DQBUF/QBUF`，当前 IMX219 使用 RG10，并由 `SoftwareIsp` 转成 BGRx。
...
`camera_driver` 内的运行期看门狗检查 preview source 是否仍在运行以及最后收帧时间，将故障区分为 `source_disconnected`、`no_frames` 和 `frame_stalled`。WSL 可将 `capture_backend` 切换为 `synthetic`，通过同一个 `CameraPreviewSource` 边界注入无帧、卡帧和断流；恢复仍使用正式 start/recover 状态机，因此合成测试与真实 Argus/V4L2 pipeline 共享指标和控制逻辑。
