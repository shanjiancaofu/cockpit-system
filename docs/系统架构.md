...
Cockpit BridgeControl gRPC
    ...
    → fake provider（当前）/ ROS2 Nav2 provider（后续）
```
...
覆盖 accepted→executing→succeeded、reject、fail、stall/timeout、cancel 和 disconnect→recover。当前没有
ROS2 node、Nav2 action client、地图、定位或底盘控制；Bridge 也不能发送 CAN 或调用 vehicle control。
