...
| 语音识别 | 部分完成 | `agent/speech/asr`、`agent/speech/pipeline`、`agent/speech/providers/sherpa` | mock ASR、分段、deadline/cancel、PCM→VAD→ASR→Agent E2E；Ubuntu x86_64 SenseVoice 真实 fixture smoke 通过；USB 录音直送 SenseVoice 可识别“打开摄像头” | 真实麦克风连续讲话经 KWS→VAD→ASR 的稳定声学验收，并在 Jetson 对比真实 ASR |
