...
| HMI 动作桥接 | 部分完成 | `agent/hmi`、`cockpit/apps/cockpit-ui/hmi_control.*` | `open_camera` Qt 主线程切页、媒体未接入失败路径、Navigator smoke | 等本地媒体 owner 落地后闭环 `play_music`；Android/投屏后端继续后置 |
...
| 用户车机 UI | 可用原型 | `cockpit/apps/cockpit-ui/qml`、Vehicle/Camera/Health/HMI/Voice/AppLauncher/Media models | 1280×720 首页、六页 Dock、真实 Voice 状态/回复/中断、固定 App/track ID、媒体控制状态机、完整 QML offscreen 和模型/gRPC 测试 | 选择真实媒体 owner/音频焦点 backend；Jetson 触摸、图形栈和长稳 |
| 本地媒体 | 部分完成 | `cockpit/modules/media`、`cockpit/library/media`、`cockpit/apps/cockpit-ui/media` | `media.proto`、Navigator media module、disabled/mock provider、真实 gRPC、固定 track ID、play/pause/resume/next/stop、UI mode 生命周期和 fixture 测试 | 将 UI model 切到 media gRPC；实现受控 manifest 与 GStreamer/fakesink backend；音频焦点后再闭环 HMI |
