...
| HMI 动作桥接 | 部分完成 | `agent/hmi`、`cockpit/apps/cockpit-ui/hmi_control.*` | `open_camera` Qt 主线程切页、媒体未接入失败路径、Navigator smoke | 重做用户车机 UI；等待 Android/投屏媒体后端后接入 App |
...
| 用户车机 UI | 可用原型 | `cockpit/apps/cockpit-ui/qml`、Vehicle/Camera/Health/HMI/Voice models | 1280×720 首页、六页 Dock、真实 Voice 状态/回复/中断、完整 QML offscreen 加载、HMI 相机切页和模型/gRPC 测试 | 实现受控 App Launcher/本地媒体后端；Jetson 触摸、图形栈和长稳 |
