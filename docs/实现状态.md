...
| 语音识别 | 部分完成 | `modules/voice/asr`、`library/driver/audio` | mock ASR tests 和 smoke | 真实 ASR 独立选型与接入 |
...
| 崩溃收集 | 已落地 | `navigator/diagnostics/crash_reporter.*`、`navigator/process/process_manager.*` | JSON 合法性、信号故障注入、成功重启、重启上限、干净关闭和最多 20 份保留测试 | 后期按隐私和后端合同决定是否上传；当前不抓 core |
