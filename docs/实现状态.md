...
| 语音识别 | 部分完成 | `agent/speech/asr`、`agent/speech/pipeline`、`agent/speech/providers/sherpa` | mock ASR、分段、deadline/cancel、PCM→VAD→ASR→Agent E2E；Ubuntu x86_64 SenseVoice 真实 fixture smoke 通过；USB 录音直送 SenseVoice 可识别“打开摄像头”；诊断支持 300 ms pre-roll 和多段 transcript | 真实麦克风连续讲话经 KWS→VAD→ASR 的稳定声学验收，并在 Jetson 对比真实 ASR |
...
| 语音播放 | 可用原型 | `agent/speech/tts`、`agent/speech/providers/sherpa`、`agent/audio`、`cockpit/library/driver/audio` | mock TTS、TTS deadline/cancel、Kokoro 中英混读 24 kHz PCM smoke、USB 扬声器回放和真实 playback receipt | Agent 动态回复分句、播放队列和 Jetson 声学标定 |
