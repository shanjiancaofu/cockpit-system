...
| 语音识别 | 部分完成 | `agent/speech/asr`、`agent/speech/pipeline`、`agent/speech/providers/sherpa` | mock ASR、分段、deadline/cancel、PCM→VAD→ASR→Agent E2E；SenseVoice provider 骨架已起步 | 在 Agent 产品构建中接入并对比真实 ASR |
| 语音唤醒/KWS | 部分完成 | `agent/runtime/voice_input_gate.*`、`agent/speech/kws`、`agent/speech/providers/sherpa` | `voice_input_gate_test`、`system_config_test`；KWS 关闭直通、Idle→KWS、Listening/FollowUp→Speech、半双工暂停、cooldown、自定义唤醒词配置和异步 wake cue | Jetson 准备 Sherpa runtime/model 后跑真实 KWS smoke 和声学误唤醒测试 |
...
| 语音播放 | 可用原型 | `agent/speech/tts`、`agent/audio`、`cockpit/library/driver/audio` | mock TTS、TTS deadline/cancel、PCM RPC、真实 playback receipt、异步播放和有界停止测试 | 接真实 TTS provider、扬声器和声学标定 |
