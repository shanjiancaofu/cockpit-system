...
| HMI 动作桥接 | 可用原型 | `agent/hmi`、`cockpit/apps/cockpit-ui/hmi_control.*` | `open_camera` Qt 主线程切页、`play_music` 固定曲目、pause/resume 音频焦点命令、mock media gRPC 成功链路和 Navigator smoke | 真实 USB 语音/音乐混合闭环；Android/投屏后端继续后置 |
...
| 本地媒体 | 可用原型 | `cockpit/modules/media`、`cockpit/library/media`、`cockpit/apps/cockpit-ui/media` | `media.proto`、Navigator media module、真实 gRPC/UI client、受控 manifest、GStreamer fakesink/EOS/error、Ubuntu USB `alsasink` smoke、HMI 固定曲目、TTS pause/resume 音频焦点 | 真实 USB 语音/音乐混合闭环；Jetson 真机扬声器和长稳仍待验收 |
