...
| HMI 动作桥接 | 部分完成 | `agent/hmi`、`cockpit/apps/cockpit-ui/hmi_control.*` | `open_camera` Qt 主线程切页、媒体 gRPC 控制面已接入 UI、Navigator smoke | 等本地媒体 owner 落地后闭环 `play_music`；Android/投屏后端继续后置 |
...
| 用户车机 UI | 可用原型 | `cockpit/apps/cockpit-ui/qml`、Vehicle/Camera/Health/HMI/Voice/AppLauncher/Media models | 1280×720 首页、六页 Dock、真实 Voice 状态/回复/中断、固定 App/track ID、媒体控制状态机、media gRPC client、完整 QML offscreen 和模型/gRPC 测试 | 选择真实媒体 owner/音频焦点 backend；Jetson 触摸、图形栈和长稳 |
| 本地媒体 | 部分完成 | `cockpit/modules/media`、`cockpit/library/media`、`cockpit/apps/cockpit-ui/media` | `media.proto`、Navigator media module、disabled/mock provider、真实 gRPC、UI gRPC client、受控 manifest、SHA-256/路径边界、GStreamer `playbin3` fakesink/EOS/error 测试、Ubuntu VM USB 声卡 `alsasink` 播放/暂停/恢复/停止 smoke、固定 track ID | 定义 Voice/Media 音频焦点并接 HMI；Jetson 真机扬声器和长稳仍待验收 |
...
这条基线表示 WSL 原型具备自动验证证据，不表示 HMI 媒体控制、云端上传、Jetson 硬件或
量产升级已经完成。
