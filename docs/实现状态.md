...
| 语音识别 | 部分完成 | `agent/speech/asr`、`agent/speech/pipeline`、`agent/speech/providers/sherpa` | mock ASR、分段、deadline/cancel、PCM→VAD→ASR→Agent E2E；Ubuntu x86_64 fixture smoke 通过；USB 麦克风真实讲话完成 VAD→SenseVoice→`open_camera` 路由，4/4 transcripts、0 errors | 真实麦克风加入 KWS 后完成连续闭环，并在 Jetson 对比真实 ASR |
...
| 语音播放 | 可用原型 | `agent/speech/tts`、`agent/speech/providers/sherpa`、`agent/audio`、`cockpit/library/driver/audio` | mock TTS、TTS deadline/cancel、Kokoro 中英混读 24 kHz PCM smoke、USB 扬声器回放、真实 playback receipt、动态回复分句逐段播放 | 播放队列扩展和 Jetson 声学标定 |
...
- PCM2902 USB audio 设备级 ALSA 3 秒录制和回放已验证；KWS/VAD/ASR 真实端到端声学链路待验证。
- 真实扬声器、AEC 和车内噪声环境。
- IMX219 小时级预览稳定性。
