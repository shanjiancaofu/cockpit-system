...
| 真实 ASR 实现 | Sherpa SenseVoiceSmall INT8 provider 已在 Ubuntu x86_64 固定录音和真实讲话 smoke 中验证；扩展标注、连续 KWS→ASR 和 Jetson smoke 待完成 |
| TTS 与 PCM 回放 | Sherpa Kokoro `kokoro-multi-lang-v1_1` 已加入显式 Agent 产品构建；Ubuntu x86_64 中英混读和 USB PCM 回放通过，默认构建仍为 mock |
| 本地 LLM client | `LocalLlmClient`、llama-server SSE、首 token/总超时、进程托管、Qwen3.5 2B/4B smoke 和 thinking 关闭已完成；Jetson 性能、并发和长稳待验收 |
...
未完成 Jetson 真实模型加载、声学 smoke 和资源验收前，不得把 Ubuntu x86_64 provider 基线伪装成
生产语音链路。
