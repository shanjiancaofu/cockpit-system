...
8. Sherpa公开C API固定兼容的WenetSpeech 2024 mobile模型，保留2025 chunk-8不兼容复现证据。
9. 音频异常采用“真实故障记录→进程恢复→重复Target/ASan/TSan回归→显式残余风险”口径，不伪造根因。
...
13. 为什么一次无法复现的SIGSEGV不能写成“已修复”，但仍可以做版本Freeze？
14. 为什么固定30秒压测产生的active-session拒绝不是服务失败？
