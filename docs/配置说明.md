...
业务配置文件是 `configs/config.yaml`，由 `cockpit/core/config/SystemConfig` 解析并在进程启动时校验。
Navigator 另用 `configs/navigator.yaml` 描述进程级模块和运行模式，不把运行编排混入业务参数。

## Navigator

```yaml
initial_mode: normal
socket_path: /tmp/cockpit-navigator.sock
...
modes:
  normal: [transfer]
  development: [transfer, recording, debugger]
```

- `initial_mode` 必须存在于 `modes`。
- module name 只能使用小写字母、数字、`_` 和 `-`，并且不能重复。
- `library` 的相对路径以 `--module-dir` 为基准。
- module `config` 的相对路径以 `navigator.yaml` 所在目录为基准。
- `startup_timeout_ms` 内必须完成动态库加载、ABI 校验和模块 `start`，否则启动失败。
- 同一 mode 不能重复引用模块，也不能引用未声明模块。
- `max_attempts` 是时间窗口内允许的自动重启次数，超过后模块进入 `failed`。
- 本地控制命令包括 `status`、`mode`、`switch`、`start`、`stop`、`restart` 和 `shutdown`。
