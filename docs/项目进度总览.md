...
| 音频语音链路 | ALSA 抽象、AudioFrame、SPSC、VAD、分段、mock ASR/TTS、可选 whisper.cpp 与 sherpa-onnx/SenseVoice adapter、停止取消链路 |
...
| 真实 ASR 基线 | whisper.cpp `small` 完成 WSL 对照；sherpa-onnx + SenseVoice INT8 在 Jetson 完成真实中文 WAV 识别，默认构建和发布包不携带模型 |
...
7. sherpa-onnx + SenseVoice INT8 已完成 Jetson CPU 基线并选为当前 provider；USB 音频模块到位后验证
   真实录音、噪声、端到端延迟和长运行内存，再决定是否需要 TensorRT 优化。
...
| 真实声学性能未知 | SenseVoice INT8 已有 Jetson 文件识别基线，但没有 USB 麦克风、车内噪声、AEC 和长运行证据 | 音频模块到位后测端到端延迟、准确率、温度、功耗和持续 RSS |
...
6. SenseVoice INT8 Jetson 文件识别已通过；接 USB 麦克风后验证真实端到端语音链路。
