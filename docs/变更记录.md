...
- `Logger`、`Time`、`Args`、`Module`、`ServiceRuntime` 和 `VehicleState` 等历史文件统一改为
  `snake_case`，同步源码、测试、构建和文档引用。
- recording event publisher 从 `services/recording-service/client` 迁移到
...
- tests 按 core、modules、services、apps、tools 和 integration 分组，继续由一个 CMake 文件集中声明。
