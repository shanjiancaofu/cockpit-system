...
## 2026-07-12 - 结构审计与 cockpit-ctl 薄入口

### 变更内容

- 审计 73 个 CMake 构建文件、主要 target 直接依赖和大文件职责，新增 `结构收口清单.md`，记录
  保留、已收口、候选和后续批次。
- `cockpit-ctl` 将进程入口与命令实现分开：`main.cc` 只调用 `ctl::Run`，现有 status、health、
  dependencies 和 watch 实现移入 `commands.cc`。
- CMake 仍只生成一个 `cockpit-ctl` executable，没有增加 library、manager、provider 或运行层级。

### 设计决定

- 大文件不按行数拆分；shared memory、recording session 和 audio processing 保持资源状态内聚。
- 小 target 不批量合并；先确认第三方依赖、编译条件、测试和当前调用方，再处理聚合 target。
- 暂停新增功能，后续依次审计配置职责、直接依赖和重复状态文档。

### 验证结果

- x86_64 Debug 构建、全量 37 项 CTest、完整 smoke 和 pre-commit 通过。
- 单独验证 help、dependencies 和离线 health JSON；输出与退出码保持不变。
