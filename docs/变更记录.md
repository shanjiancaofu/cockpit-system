...
- deploy 源文件迁入 `scripts/deploy`，发布包内部目录和安装合同保持不变。
...
- `bash scripts/run_smoke.sh` 通过，覆盖 recording、topic、audio、voice、camera 和 cloud placeholder。
...
- `scripts/deploy/THIRD_PARTY_NOTICES.md` 属于第三方许可证说明，保持上游原文。
...
- `pre-commit run --files cockpit/core/logging/logger.h scripts/run_tidy.sh` 通过。
- `bash scripts/build.sh` 通过，CTest 17/17。
