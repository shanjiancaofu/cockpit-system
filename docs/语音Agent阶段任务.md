...
- [x] 唤醒提示与 TTS 失败提示使用无模型固定 PCM，直接走 Audio Playback。
...
- [x] 固定提示不依赖 TTS 模型。
- [x] Ubuntu x86_64 固定基准确认动态回复可生成中文、英文和中英混读 24 kHz mono PCM；主观自然度
  仍需现场验收。
- [x] 空文本和过期 deadline 明确失败，异常符号成功或明确诊断，长文本不会导致崩溃；真实取消后
  同一 Kokoro provider 可恢复。
- [x] Ubuntu x86_64 服务级 8 轮顺序播放无明显内存增长；第 3 轮 warm-up 后服务进程树 RSS 增长
  276 KiB，8 次 playback receipt 全部完成。并发队列和 Jetson 长稳仍未验收。
- [x] TTS 失败时回退到一次固定 PCM 提示，并停止剩余文本分段。
...
- [x] Ubuntu x86_64 同一 Kokoro provider 真实重复合成/RSS 基线；默认 16 轮且可显式增加轮数，
  warm-up 后增长有界；服务级真实 ALSA 顺序播放 8 轮也已通过。Jetson 和并发队列长稳仍未验收。
- [x] Kokoro 为离线整段 API，不存在流式首包；Ubuntu 固定基准记录首个完整分段可播放延迟，长回复
  平均从整段 5826 ms 降至首段 1126 ms。
- [x] Ubuntu x86_64 固定文本基准记录平均/P95/RTF；18 个样本整体平均 2566 ms、观测 P95
  5888 ms、平均 RTF 0.645。Jetson 指标仍未验收。
- [ ] 中文自然度。
- [x] 中英混读可生成有效 PCM；主观发音质量仍待现场验收。
- [x] 数字和单位固定文本可生成有效 PCM；读法正确性仍待人工听测。
- [x] 空文本、过期 deadline、异常符号、长文本和取消恢复基线通过。
- [x] Ubuntu 同 provider 32 轮和服务级 ALSA 8 轮内存增长有界；Jetson 长稳仍未验收。
