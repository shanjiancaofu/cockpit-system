...
长运行产品进程由 Navigator 统一管理。systemd target 只选择 mode 并守护一个
`cockpit-navigator@<mode>.service`；Navigator 通过 `fork + exec`、版本化 `dlopen` ABI 和代码内
`run_config` 管理 module child 的启停、模式切换、状态、崩溃重启上限和退出回收。
