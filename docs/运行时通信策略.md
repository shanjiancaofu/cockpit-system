...
它不经过 transfer 模块，连接、读写和服务端回复均有 1 秒上限。`ProcessRuntime` 和
...
YAML 中的 `runtime.dependencies` 是旧独立 service 架构遗留的展示副本，只被
`cockpit-ctl dependencies` 读取，不参与 Navigator 启动顺序、mode 或重启。当前权威运行策略是
`navigator/run_config/run_config.cc`；该重复配置将在 WSL-R2 清理。
